...
const AMOVIESETUP_FILTER AMSFilterVCam =
{
	...
	L"Kinect Cam with zoom",     // String name
...
STDAPI RegisterFilters(BOOL bRegister)
{
	...
	if (bRegister)
	{
		hr = AMovieSetupRegisterServer(CLSID_VirtualCam, L"Kinect Cam with zoom", achFileName, L"Both", L"InprocServer32");
	}
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			if (bRegister)
			{
				...
				hr = fm->RegisterFilter(CLSID_VirtualCam, L"Kinect Cam with zoom", &pMoniker, &CLSID_VideoInputDeviceCategory, nullptr, &rf2);
			}
			else
			{
				hr = fm->UnregisterFilter(&CLSID_VideoInputDeviceCategory, nullptr, CLSID_VirtualCam);
			}
		}
